...
export const getLeadReportAction = async (csrfToken, body) => {
  ...
  try {
    ...
    const res = await axios.post(
      ...
      `${process.env.BACKEND_URL_DEV}/leads_report/all_lead_report_list`,
      body,
      {
        headers: {
...
export const uploadLeadReportAction = async (csrfToken, body) => {
  const cookieStore = cookies();
  const sessionToken = cookieStore.get("sessionToken");
  // const bToken = cookieStore.get("token");
  ...
  try {
    ...
    const res = await axios.post(
      // `${process.env.BACKEND_UAT_URL}/api/users`,
      `${process.env.BACKEND_URL_DEV}/leads_report/update_lead_report`,
      body,
      {
        headers: {
          Authorization: `Bearer ${bToken?.value}`,
