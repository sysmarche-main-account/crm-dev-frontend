...
import {
  ...
  getSingleRuleDataAction,
} from "@/app/actions/ruleActions";
import { decryptClient } from "@/utils/decryptClient";
import { getToken } from "@/utils/getToken";

const EditRules = ({ onClose, rule, open, handleDataChange }) => {
  const logout = useLogout();
  const { showSnackbar } = useSnackbar();
  const t = useTranslations();

  const formikEditRuledRef = useRef();

  const editLeadRuleValidationSchema = Yup.object({
    name: Yup.string()
      .min(3)
      .max(200)
    ...
    // Change source validation to accept an array
    source: Yup.array()
  ...
  const getSelectedRuleData = async () => {
    ...
    try {
      const result = await getSingleRuleDataAction(csrfToken, reqbody);
      ...
      if (result.success && result.status === 200) {
        ...
        console.log("final rule data", decrypted);

        setRuleData(decrypted);
        setSelectedUni(decrypted?.university?.id);
        setSelectedPrgm(decrypted?.program?.id);
        setSelectedAgent(decrypted?.agent);
  ...
  const handleEditRuleSubmit = async (values) => {
    ...
    // remove null/undefined values from array but allow 0
    const courseValues = values.course.filter(
      (c) => c !== null && c !== undefined
    );
    const sourceValues = values.source.filter(
    ...
    const reqbody = {
      ...
      source: sourceValues, // refer from master
      course: courseValues, //refer from master
      ...
      if (result.success && result.status === 200) {
        ...
        showSnackbar({
          message: `<strong>${decrypted.rule_name}</strong> ${t(
            "rules.rule_editrule_alert"
          )}`,
          severity: "success",
          anchorOrigin: { vertical: "top", horizontal: "center" },
        });
        handleDataChange();
        formikEditRuledRef.current.resetForm();
      } else {
        console.error(result.error);
        if (result.error.status === 500) {
          await logout();
  ...
  return (
    <div className="map-roles-modal-user" id="map-roles-modal-user">
      <div className="modal-header-roles" id="modal-header-roles">
        <h2 id="modal-header-title">{t("rules.rule_editrule")}</h2>
        <div
          id="manage-rules-edit-close-btn"
      ...
      {loading?.data ? (
        ...
        <div className="modal-body" id="modal-body">
          <p id="rules-subtitle">{t("rules.rules_createform_subtitle")}</p>
          <div
            className="modal-content-user-section"
            id="modal-content-user-section"
          >
            <Formik
              innerRef={formikEditRuledRef}
              initialValues={{
                name: ruleData?.rule_name || "",
                university: ruleData?.university?.id || "",
                // program: ruleData?.program?.id || "",
                course:
                  ruleData?.course?.[0] === 0
                    ? [0]
                    : ruleData?.course?.map((course) => course?.id) || [],
                source:
                  ruleData?.source?.[0] === 0
                    ? [0]
                    : ruleData?.source?.map((source) => source?.id) || [],
                start: ruleData?.start_time || "",
                end: ruleData?.end_time || "",
                user_id:
                  ruleData?.counsellor?.map((counsel) => counsel?.uuid) || [],
                day: ruleData?.day?.map((day) => day?.id) || [],
                agent: ruleData?.agent || "",
                order: ruleData?.sorting_order || "",
              }}
              validationSchema={editLeadRuleValidationSchema}
              ...
              {({ setFieldValue, touched, errors, values }) => (
                <>
                  ...
                  >
                    ...
                    {/* university */}
                    <FormControl fullWidth id="university-form-control">
                      <label className="role-label" htmlFor="univesity">
                        {t("manage_user.mu_eum_assigne_univ")}
                        <span style={{ color: "red" }}>*</span>
                      </label>
                      <Select
                        id="manage-rules-edit-university"
                    ...
                    {/* new  course*/}
                    <FormControl fullWidth id="course-form-control">
                      <label className="role-label" htmlFor="course">
                        {t("rules.rules_create_course")}
                        <span style={{ color: "red" }}>*</span>
                      </label>
                      ...
                      <Select
                        ...
                        multiple
                        value={
                          Array.isArray(values.course) ? values.course : []
                        }
                        disabled={!Boolean(selectedUni)}
                        onChange={(e) => {
                      ...
                      >
                        ...
                        {courses?.length > 1 && (
                          <MenuItem value="all">
                            <Checkbox
                              id="manage-rules-edit-all-courses"
                              checked={
                                values.course?.includes(0) ||
                                values.course?.length === courses?.length
                              }
                        ...
                        {loading.course ? (
                          ...
                          courses?.map((course) => (
                            <MenuItem key={course.id} value={course.id}>
                              <Checkbox
                                id={`manage-rules-edit-course-${course.id}`}
                                checked={
                                  values.course.includes(0) ||
                    ...
                    {/* source */}
                    <FormControl fullWidth id="source-form-control">
                      <label className="role-label" htmlFor="source">
                        {t("rules.rule_managerule_source")}
                        <span style={{ color: "red" }}>*</span>
                      </label>
                      <Select
                        id="manage-rules-edit-source"
                        multiple
                        value={
                          Array.isArray(values.source) ? values.source : []
                        }
                        onChange={(e) => {
                          const selectedValues = e.target.value;
                          if (selectedValues.includes("all")) {
                      ...
                      >
                        ...
                        {channelList?.length > 1 && (
                          <MenuItem value="all">
                            <Checkbox
                              id="manage-rules-edit-all-sources"
                              checked={
                                values.source?.includes(0) ||
                                values.source?.length === channelList?.length
                              }
                        ...
                        {loading.channel ? (
                          ...
                          channelList?.map((ch) => (
                            <MenuItem key={ch.id} value={ch.id}>
                              <Checkbox
                                id={`manage-rules-edit-source-${ch.id}`}
                                checked={
                                  values.source.includes(0) ||
                    ...
                    {/* time */}
                    <p id="daytime-select-text">
                      {t("rules.rules_create_daytime_select")}
                    </p>
                    <FormControl fullWidth id="time-form-control">
                      <div
                        style={{ display: "flex", gap: "10px" }}
                        id="time-inputs-container"
                      >
                        {/* Start Time */}
                        <div style={{ width: "50%" }} id="start-time-container">
                          <label className="role-label" htmlFor="start">
                            {t("rules.rule_create_starttime")}{" "}
                            <span style={{ color: "red" }}>*</span>
                          </label>
                          <Field
                            id="manage-rules-edit-start-time"
                            type="time"
                            name="start"
                          />
                        ...
                        {/* End Time */}
                        <div style={{ width: "50%" }} id="end-time-container">
                          <label className="role-label" htmlFor="end">
                            {t("rules.rule_create_end_time")}{" "}
                            <span style={{ color: "red" }}>*</span>
                          </label>
                          <Field
                            id="manage-rules-edit-end=time"
                            type="time"
                            name="end"
                          />
                    ...
                    {/* Day */}
                    <FormControl fullWidth id="day-form-control">
                          ...
                          days.map((day) => {
                            return (
                              <MenuItem key={day.id} value={day.id}>
                                <Checkbox
                                  checked={values.day.includes(day.id)}
                                />
                    ...
                    {/* Assignees(Agent) */}
                    <p id="assign-to-text">{t("rules.rule_create_assignto")}</p>
                    <FormControl fullWidth id="agent-form-control">
                      ...
                      >
                        <FormControlLabel
                          ...
                          control={
                            <Radio
                              id="manage-rules-edit-digital-agent"
                              sx={{
                                color: "#D9D9D9",
                                "&.Mui-checked": {
                                  color: "#007143",
                        ...
                        <FormControlLabel
                          ...
                          control={
                            <Radio
                              id="manage-rules-edit-human-agent"
                              sx={{
                                color: "#D9D9D9",
                                "&.Mui-checked": {
                                  color: "#007143",
                    ...
                    {selectedAgent === 1 && (
                      ...
                      //   <Select
                      //     id="manage-rules-edit-counsellors"
                      //     displayEmpty
                      //     renderValue={(selected) => {
                      ...
                      //       //       ? `${t(
                      //       //           "manage_template.mt_selec_counsellors"
                      //       //         )}`
                      //       //       : selected
                      //       //           ?.map(
                      //       //             (id) =>
                      ...
                      //             <Checkbox
                      //               id={`manage-rules-edit-checkbox-${item?.uuid}`}
                      ...
                      //   />
                      // </FormControl>
                      <FormControl fullWidth id="counsellors-form-control">
                        <label className="role-label" htmlFor="user_id">
                          {t("rules.rule_create_selec_consull")}
                          <span style={{ color: "red" }}>*</span>
                        </label>
                    ...
                    {/* rule order */}
                    <FormControl fullWidth id="order-form-control">
                      <label className="role-label" htmlFor="order">
                        {t("rules.rule_create_edit_order")}
                      </label>
                      <Field
                        id="manage-rules-edit-rule-order"
                        type="number"
                        name="order"
                        min={1}
                        max={999}
                  ...
                  <div
                    ...
                    id="create-rule-alert-text"
                  >
                    <p style={{ width: "8%" }} id="alert-icon-container">
                      <AlertIcon />
                    </p>
                    <h4 id="alert-text">
                      {t("rules.rule_create_heading_roundrobin")}
                    </h4>
                  </div>
                </>
              )}
      ...
      <div className="modal-footer" id="modal-footer">
        <Button
          id="manage-rules-edit-cancel-btn"
          variant="outlined"
          onClick={onClose}
        ...
        <div style={{ display: "flex" }} id="submit-button-container">
          <Button
            id="manage-rules-edit-submit-btn"
            variant="contained"
            color="success"
            className="map-role-button"
            onClick={() => formikEditRuledRef.current.submitForm()}
