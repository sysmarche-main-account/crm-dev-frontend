...
const CreateEmailTemplate = ({ open, onClose, handleDataChange }) => {
  ...
  const formikCreateTemplatedRef = useRef();
  const fileInputRef = useRef(null);

  const stripHtml = (html) => html.replace(/<[^>]*>/g, "").trim();

  const createEmailTemplateValidationSchema = Yup.object({
    title: Yup.string()
      .min(3)
      .max(200)
  ...
  const handleCreateTemplate = async (values) => {
    ...
    formData.append("template_status", 0);
    ...
    try {
      const result = await createTemplateAction(csrfToken, formData);
      ...
      if (result.success && result.status === 200) {
        ...
        setFiles(decrypted?.attachments);
        setIsDraft(true);
        handleDataChange();
  ...
  const handleCreateTemplatePublish = async (values) => {
    console.log("values", values);
  ...
  const handleUpdateTemplate = async (values) => {
    ...
    const csrfToken = await getCsrfToken();
    const formData = new FormData();
    formData.append("template_ref_id", templateInfo?.template_ref_id);
    formData.append("template_cat", "email");
    formData.append("subject", values?.subject);
    formData.append("template_name", values?.title);
    values?.counsellors?.forEach((counselor) => {
    ...
    // const reqbody = {
    //   template_ref_id: templateInfo?.template_ref_id,
    //   template_cat: "email", //email|sms|whatsapp| Notifications // pass by default within form body
    //   subject: templateInfo?.subject, //Optional if identfier is email then manditory
    //   // dlt_template_id: "21334536565", //Optional if identfier is SMS then manditory
    //   template_name: templateInfo?.title, //Mandatory
    //   visible_for: templateInfo?.counsellors, // 0 - All counsellor 0
    //   body_content: templateInfo?.body, //String value and mandatory
    //   template_status: 1,
    //   //"template_status":1 //optional pass by default 1
    // };
    ...
    try {
      ...
      if (result.success && result.status === 200) {
        ...
        handleDataChange();
        formikCreateTemplatedRef.current.resetForm();
      } else {
        console.error(result.error);
        if (result.error.status === 500) {
          await logout();
    ...
    <>
      <div id="email-template-modal" className="email-template-modal">
        <div id="go-back-section" className="go-back-section">
          <div
            id="template-email-create-go-back-btn"
        ...
        <div id="modal-header" className="modal-header">
          <h2>{t("manage_template.mt_create_new")}</h2>
        </div>
        <div
        ...
        >
          <Formik
            innerRef={formikCreateTemplatedRef}
            initialValues={{
              title: "",
              subject: "",
              counsellors: [],
            ...
            validationSchema={createEmailTemplateValidationSchema}
            onSubmit={(values) => {
              if (values.action === "draft" && !isDraft && !templateInfo) {
                console.log("new", "draft", isDraft);
                ...
                handleCreateTemplatePublish(values);
              } else if (
                isDraft &&
                templateInfo &&
                values.action === "publish"
                ...
                <Form>
                  ...
                  >
                    ...
                    <div id="form-group-title" className="form-group">
                      ...
                      <Field
                        id="template-email-create-title"
                        type="text"
                        name="title"
                    ...
                    {/* subject */}
                    <div id="form-group-subject" className="form-group">
                      <label className="role-label" htmlFor="subject">
                        {t("manage_template.et_subject")}{" "}
                        <span style={{ color: "red" }}>*</span>{" "}
                      </label>
                      <Field
                        id="template-email-create-subject"
                        type="text"
                        name="subject"
                    ...
                    {/* variable */}
                    <div id="form-group-variable" className="form-group">
                      <label className="role-label">
                        {t("manage_template.csmst_variable_lab")}
                      </label>
                      <Select
                        id="template-email-create-variable"
                        name="variable"
                    ...
                    {/* visible_for */}
                    <div id="form-group-counsellors" className="form-group">
                      <label className="role-label" htmlFor="counsellors">
                        {t("manage_template.cwam_visible_for")}{" "}
                        <span style={{ color: "red" }}>*</span>
                      </label>
                      <Select
                        id="template-email-create-visible-for"
                        displayEmpty
                  ...
                  {/* body */}
                  <div id="form-group-body" className="form-group">
                    <label htmlFor="body">
                      {t("manage_template.mt_compose_email")}{" "}
                      <span style={{ color: "red" }}>*</span>{" "}
                    </label>
                    <SummerNoteEditor
                      id="template-email-create-body"
                      value={value}
                      onChange={(newContent) => {
                  ...
                  >
                    ...
                    >
                      <Button
                        id="template-email-create-file-attach-btn"
                        variant="outlined"
                    ...
                    >
                      {isDraft && !loading.draft ? (
                        <Button
                          id="template-email-create-test-modal-btn"
                          variant="contained"
                          color="primary"
                          className="save-create-btn"
                        ...
                        <Button
                          id="template-email-create-draft-btn"
                          type="submit"
                          variant="contained"
                          color="primary"
                          className="save-create-btn"
                        ...
                        >
                          {loading.draft ? (
                            ...
                            t("manage_template.mt_btn_save_draft")
                          )}
                        </Button>
                      )}

                      <Button
                        id="template-email-create-submit-btn"
                        type="submit"
                        variant="contained"
                        color="primary"
                        className="save-btn"
