...
const EditSubStatus = ({ data, onClose, onSave }) => {
  const t = useTranslations("edit_substatus");
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (result.success && result.status === 200) {
        ...
        handleApiError(result.error, "Edit SubStatus");
      }
    } catch (error) {
      console.error("❌ Unexpected Error:", error);
      handleApiError(error, "Edit SubStatus");
    } finally {
  ...
  const handleApiError = (error, context) => {
    ...
    if (error?.status === 500) {
      showSnackbar({
        message: `${context} - Internal Server Error (500). Please try again later.`,
        severity: "error",
      ...
      showSnackbar({
        message: `${context} - Unknown error type or null response.`,
        severity: "error",
        anchorOrigin: { vertical: "top", horizontal: "right" },
      });
    }
                ...
                <div className="form-group" style={{ marginBottom: "24px" }}>
                  <FormControl fullWidth variant="outlined">
                    ...
                    <Field
                      as="input"
                      name="description"
                      placeholder="Enter sub status description"
                      style={{
                        width: "730px",
                        maxWidth: "100%",
                        padding: "10px",
